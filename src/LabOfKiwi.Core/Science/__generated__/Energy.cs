...
{
    private static readonly Dimension s_dimension = new(time: -2, length: 2, mass: 1);
    ...
    public readonly struct Measurement : IMeasurement, IComparable<Measurement>, IEquatable<Measurement>
    {
        ...
        public static Voltage.Measurement operator /(Measurement left, ElectricCharge.Measurement right) => new(left.Value / right.Value);

        public static Length.Measurement operator /(Measurement left, Force.Measurement right) => new(left.Value / right.Value);

        public static Power.Measurement operator *(Measurement left, Frequency.Measurement right) => new(left.Value * right.Value);

        public static Force.Measurement operator /(Measurement left, Length.Measurement right) => new(left.Value / right.Value);

        public static Speed.Measurement operator /(Measurement left, Momentum.Measurement right) => new(left.Value / right.Value);

        public static Time.Measurement operator /(Measurement left, Power.Measurement right) => new(left.Value / right.Value);

        public static Volume.Measurement operator /(Measurement left, Pressure.Measurement right) => new(left.Value / right.Value);

        public static Momentum.Measurement operator /(Measurement left, Speed.Measurement right) => new(left.Value / right.Value);

        public static Power.Measurement operator /(Measurement left, Time.Measurement right) => new(left.Value / right.Value);

        public static ElectricCharge.Measurement operator /(Measurement left, Voltage.Measurement right) => new(left.Value / right.Value);

        public static Pressure.Measurement operator /(Measurement left, Volume.Measurement right) => new(left.Value / right.Value);
        #endregion
    }
}

...
{
    private static readonly Dimension s_dimension = new(time: 1, electricCurrent: 1);
    ...
    public readonly struct Measurement : IMeasurement, IComparable<Measurement>, IEquatable<Measurement>
    {
        ...
        public static Voltage.Measurement operator /(Measurement left, ElectricalCapacitance.Measurement right) => new(left.Value / right.Value);

        public static Time.Measurement operator /(Measurement left, ElectricCurrent.Measurement right) => new(left.Value / right.Value);

        public static ElectricCurrent.Measurement operator *(Measurement left, Frequency.Measurement right) => new(left.Value * right.Value);

        public static ElectricCurrent.Measurement operator /(Measurement left, Time.Measurement right) => new(left.Value / right.Value);

        public static Energy.Measurement operator *(Measurement left, Voltage.Measurement right) => new(left.Value * right.Value);

        public static ElectricalCapacitance.Measurement operator /(Measurement left, Voltage.Measurement right) => new(left.Value / right.Value);
        #endregion
    }
}

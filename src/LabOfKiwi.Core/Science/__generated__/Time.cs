...
{
    private static readonly Dimension s_dimension = new(time: 1);
    ...
    public readonly struct Measurement : IMeasurement, IComparable<Measurement>, IEquatable<Measurement>
    {
        ...
        public static Frequency.Measurement operator /(double left, Measurement right) => new(left / right.Value);

        public static Speed.Measurement operator *(Measurement left, Acceleration.Measurement right) => new(left.Value * right.Value);

        public static ElectricCharge.Measurement operator *(Measurement left, ElectricCurrent.Measurement right) => new(left.Value * right.Value);

        public static Momentum.Measurement operator *(Measurement left, Force.Measurement right) => new(left.Value * right.Value);

        public static Energy.Measurement operator *(Measurement left, Power.Measurement right) => new(left.Value * right.Value);

        public static Length.Measurement operator *(Measurement left, Speed.Measurement right) => new(left.Value * right.Value);
        #endregion
    }
}

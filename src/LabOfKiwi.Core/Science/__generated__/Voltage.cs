...
{
    private static readonly Dimension s_dimension = new(time: -3, length: 2, mass: 1, electricCurrent: -1);
    ...
    public readonly struct Measurement : IMeasurement, IComparable<Measurement>, IEquatable<Measurement>
    {
        ...
        public static ElectricCharge.Measurement operator *(Measurement left, ElectricalCapacitance.Measurement right) => new(left.Value * right.Value);

        public static Energy.Measurement operator *(Measurement left, ElectricCharge.Measurement right) => new(left.Value * right.Value);

        public static Power.Measurement operator *(Measurement left, ElectricCurrent.Measurement right) => new(left.Value * right.Value);
        #endregion
    }
}

...
{
    private static readonly Dimension s_dimension = new(time: -3, length: 2, mass: 1);
    ...
    public readonly struct Measurement : IMeasurement, IComparable<Measurement>, IEquatable<Measurement>
    {
        ...
        public static Momentum.Measurement operator /(Measurement left, Acceleration.Measurement right) => new(left.Value / right.Value);

        public static Voltage.Measurement operator /(Measurement left, ElectricCurrent.Measurement right) => new(left.Value / right.Value);

        public static Frequency.Measurement operator /(Measurement left, Energy.Measurement right) => new(left.Value / right.Value);

        public static Speed.Measurement operator /(Measurement left, Force.Measurement right) => new(left.Value / right.Value);

        public static Energy.Measurement operator /(Measurement left, Frequency.Measurement right) => new(left.Value / right.Value);

        public static Acceleration.Measurement operator /(Measurement left, Momentum.Measurement right) => new(left.Value / right.Value);

        public static Force.Measurement operator /(Measurement left, Speed.Measurement right) => new(left.Value / right.Value);

        public static Energy.Measurement operator *(Measurement left, Time.Measurement right) => new(left.Value * right.Value);

        public static ElectricCurrent.Measurement operator /(Measurement left, Voltage.Measurement right) => new(left.Value / right.Value);
        #endregion
    }
}

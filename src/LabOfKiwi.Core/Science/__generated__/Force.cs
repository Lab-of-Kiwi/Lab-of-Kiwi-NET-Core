...
{
    private static readonly Dimension s_dimension = new(time: -2, length: 1, mass: 1);
    ...
    public readonly struct Measurement : IMeasurement, IComparable<Measurement>, IEquatable<Measurement>
    {
        ...
        public static Mass.Measurement operator /(Measurement left, Acceleration.Measurement right) => new(left.Value / right.Value);

        public static Pressure.Measurement operator /(Measurement left, Area.Measurement right) => new(left.Value / right.Value);

        public static Momentum.Measurement operator /(Measurement left, Frequency.Measurement right) => new(left.Value / right.Value);

        public static Energy.Measurement operator *(Measurement left, Length.Measurement right) => new(left.Value * right.Value);

        public static Acceleration.Measurement operator /(Measurement left, Mass.Measurement right) => new(left.Value / right.Value);

        public static Frequency.Measurement operator /(Measurement left, Momentum.Measurement right) => new(left.Value / right.Value);

        public static Area.Measurement operator /(Measurement left, Pressure.Measurement right) => new(left.Value / right.Value);

        public static Power.Measurement operator *(Measurement left, Speed.Measurement right) => new(left.Value * right.Value);

        public static Momentum.Measurement operator *(Measurement left, Time.Measurement right) => new(left.Value * right.Value);
        #endregion
    }
}

...
{
    private static readonly Dimension s_dimension = new(time: -1, length: 1, mass: 1);
    ...
    public readonly struct Measurement : IMeasurement, IComparable<Measurement>, IEquatable<Measurement>
    {
        ...
        public static Power.Measurement operator *(Measurement left, Acceleration.Measurement right) => new(left.Value * right.Value);

        public static Time.Measurement operator /(Measurement left, Force.Measurement right) => new(left.Value / right.Value);

        public static Force.Measurement operator *(Measurement left, Frequency.Measurement right) => new(left.Value * right.Value);

        public static Speed.Measurement operator /(Measurement left, Mass.Measurement right) => new(left.Value / right.Value);

        public static Energy.Measurement operator *(Measurement left, Speed.Measurement right) => new(left.Value * right.Value);

        public static Mass.Measurement operator /(Measurement left, Speed.Measurement right) => new(left.Value / right.Value);

        public static Force.Measurement operator /(Measurement left, Time.Measurement right) => new(left.Value / right.Value);
        #endregion
    }
}

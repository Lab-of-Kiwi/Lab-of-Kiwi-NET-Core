...
{
    private static readonly Dimension s_dimension = new(mass: 1);
    ...
    public readonly struct Measurement : IMeasurement, IComparable<Measurement>, IEquatable<Measurement>
    {
        ...
        public static Force.Measurement operator *(Measurement left, Acceleration.Measurement right) => new(left.Value * right.Value);

        public static Volume.Measurement operator /(Measurement left, Density.Measurement right) => new(left.Value / right.Value);

        public static Momentum.Measurement operator *(Measurement left, Speed.Measurement right) => new(left.Value * right.Value);

        public static Density.Measurement operator /(Measurement left, Volume.Measurement right) => new(left.Value / right.Value);
        #endregion
    }
}

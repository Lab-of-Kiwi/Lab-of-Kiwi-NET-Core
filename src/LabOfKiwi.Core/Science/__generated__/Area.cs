...
{
    private static readonly Dimension s_dimension = new(length: 2);
    ...
    public readonly struct Measurement : IMeasurement, IComparable<Measurement>, IEquatable<Measurement>
    {
        ...
        public Length.Measurement Sqrt() => new(Math.Sqrt(Value));

        public static Volume.Measurement operator *(Measurement left, Length.Measurement right) => new(left.Value * right.Value);

        public static Length.Measurement operator /(Measurement left, Length.Measurement right) => new(left.Value / right.Value);

        public static Force.Measurement operator *(Measurement left, Pressure.Measurement right) => new(left.Value * right.Value);
        #endregion
    }
}

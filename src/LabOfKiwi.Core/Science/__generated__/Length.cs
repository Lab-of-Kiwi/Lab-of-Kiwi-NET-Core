...
{
    private static readonly Dimension s_dimension = new(length: 1);
    ...
    public readonly struct Measurement : IMeasurement, IComparable<Measurement>, IEquatable<Measurement>
    {
        ...
        public Area.Measurement Square() => new(Value * Value);

        public Volume.Measurement Cube() => new(Value * Value * Value);

        public static Volume.Measurement operator *(Measurement left, Area.Measurement right) => new(left.Value * right.Value);

        public static Energy.Measurement operator *(Measurement left, Force.Measurement right) => new(left.Value * right.Value);

        public static Speed.Measurement operator *(Measurement left, Frequency.Measurement right) => new(left.Value * right.Value);

        public static Area.Measurement operator *(Measurement left, Length.Measurement right) => new(left.Value * right.Value);

        public static Time.Measurement operator /(Measurement left, Speed.Measurement right) => new(left.Value / right.Value);

        public static Speed.Measurement operator /(Measurement left, Time.Measurement right) => new(left.Value / right.Value);
        #endregion
    }
}
